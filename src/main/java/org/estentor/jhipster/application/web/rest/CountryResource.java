...
import org.estentor.jhipster.application.repository.CountryRepository;
import org.estentor.jhipster.application.repository.search.CountrySearchRepository;
...
public class CountryResource {
    ...
    public List<Country> getAllCountries(@RequestParam(required = false) String filter) {
        if ("persons-is-null".equals(filter)) {
            log.debug("REST request to get all Countrys where persons is null");
            return StreamSupport
                .stream(countryRepository.findAll().spliterator(), false)
                .filter(country -> country.getPersons() == null)

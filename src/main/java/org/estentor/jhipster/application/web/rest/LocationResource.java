...
import org.estentor.jhipster.application.domain.Location;
import org.estentor.jhipster.application.repository.LocationRepository;
import org.estentor.jhipster.application.repository.search.LocationSearchRepository;
...
public class LocationResource {
    ...
    public List<Location> getAllLocations(@RequestParam(required = false) String filter) {
        if ("persons-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where persons is null");
            return StreamSupport
                .stream(locationRepository.findAll().spliterator(), false)
                .filter(location -> location.getPersons() == null)
